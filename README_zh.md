...
- 配置文件存储在项目目录（`.claude-code-router/`）而不是用户主目录
- 服务运行时会启用热重载（源文件更改时自动重启）
- **端口隔离**: 默认使用端口 3457，避免与生产模式（3456）冲突
- **智能服务管理**: 重启前自动关闭之前的服务
- 状态输出中会显示开发模式标识
- 您可以使用 `config.dev.example.json` 作为开发配置的起点
